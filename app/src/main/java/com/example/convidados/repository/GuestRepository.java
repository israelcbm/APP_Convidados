    ...
    public GuestModel load(int id) {
        try {
            ...
            if (cursor != null && cursor.getCount() > 0) {
                ...
                String name = cursor.getString(cursor.getColumnIndexOrThrow(DataBaseConstants.GUEST.COLUMNS.NAME));
                int presence = cursor.getInt(cursor.getColumnIndexOrThrow(DataBaseConstants.GUEST.COLUMNS.PRESENCE));
